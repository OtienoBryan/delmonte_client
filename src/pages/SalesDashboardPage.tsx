...
const SalesDashboardPage: React.FC = () => {
  ...
  // Checked-in Team Leaders modal
  const [activeLeadersModalOpen, setActiveLeadersModalOpen] = useState(false);
  const [activeTeamLeaders, setActiveTeamLeaders] = useState<Array<{
    id: number; name: string; email?: string;
    route_name?: string; region_name?: string; checkInTime?: string; checkoutTime?: string;
  }>>([]);
  ...
  const fetchActiveSalesReps = useCallback(async () => {
    ...
    try {
      const result = await fetchData('/dashboard/checked-in-sales-reps', { role: 'fmr' });
      if (result.success && result.data) {
  ...
  const fetchActiveTeamLeaders = useCallback(async () => {
    setLoadingActiveLeaders(true);
    try {
      const result = await fetchData('/dashboard/checked-in-sales-reps', { role: 'leader' });
      if (result.success && result.data) {
        const leaders = Array.isArray(result.data) ? result.data : [];
        setActiveTeamLeaders(
      ...
      setLoadingActiveLeaders(false);
    }
  }, [fetchData]);

  const openActiveLeadersModal = useCallback(() => {
    setActiveLeadersModalOpen(true);
  ...
  const navGroups = useMemo(() => [
    {
      label: 'People',
      items: [
        { to: '/sales-reps', label: 'FMRs', icon: <UsersIcon className="h-4 w-4" />, color: 'bg-blue-50 text-blue-700 border-blue-100 hover:bg-blue-100' },
        { to: '/activity', label: 'Activity', icon: <ActivityIcon className="h-4 w-4" />, color: 'bg-gray-50 text-gray-700 border-gray-100 hover:bg-gray-100' },
        { to: '/dashboard/route-compliance', label: 'FMR Compliance', icon: <CheckCircle2Icon className="h-4 w-4" />, color: 'bg-lime-50 text-lime-700 border-lime-100 hover:bg-lime-100' },
        { to: '/sales-rep-leaves', label: 'FMR Leaves', icon: <CalendarIcon className="h-4 w-4" />, color: 'bg-green-50 text-green-700 border-green-100 hover:bg-green-100', badge: pendingLeavesCount },
        ...
        { to: '/sales-rep-attendance', label: 'FMR Attendance', icon: <BarChart3Icon className="h-4 w-4" />, color: 'bg-teal-50 text-teal-700 border-teal-100 hover:bg-teal-100' },
      ],
    },
    {
      label: 'Operations',
      ...
      <div className="w-full px-3 sm:px-4 lg:px-6 py-3 sm:py-4 space-y-4 sm:space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 sm:gap-3">
          ...
          <StatCard
            ...
            value={`${stats.checkedInTeamLeaders || 0} / ${stats.activeTeamLeaders || 0}`}
            icon={<UsersIcon className="h-4 w-4 text-white" />}
            suffix={`${checkedInLeadersPct}% with active teams`}
            gradient="bg-gradient-to-br from-blue-500 to-blue-700"
            onClick={openActiveLeadersModal}
          />
          <StatCard
            title="Checked-In FMRs"
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-3 sm:p-4">
          <div className="flex items-center justify-between mb-2 sm:mb-3">
            <h2 className="text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest">Quick Access</h2>
            <Link
              to="/activity"
              className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-medium bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
              title="View Activity"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-3 sm:p-4 lg:p-5">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 gap-2 sm:gap-0">
              <div className="flex-1">
                <h2 className="text-xs sm:text-sm font-bold text-gray-900">Top 10 FMRs</h2>
            ...
            {topReps.length > 0 ? (
              ...
              <div className="h-52 flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="text-xs">No FMR data available</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeLeadersModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4">
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden" style={{ maxHeight: '95vh' }}>
            ...
            {/* Header */}
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-blue-600 to-indigo-600 flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <h2 className="text-xs sm:text-sm font-bold text-white truncate">Checked-In Team Leaders</h2>
                  <p className="text-[9px] sm:text-[10px] text-blue-200 truncate">
                    {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                  </p>
                </div>
              </div>
              <button
                onClick={() => setActiveLeadersModalOpen(false)}
            ...
            {/* Summary strip */}
            <div className="flex items-center justify-between px-4 sm:px-6 py-2.5 sm:py-3 bg-blue-50 border-b border-blue-100 flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                <span className="text-sm sm:text-base font-bold text-blue-800 leading-none">{activeTeamLeaders.length}</span>
                <span className="text-[10px] sm:text-xs text-blue-600">Team leaders checked in today</span>
                {stats.activeTeamLeaders > 0 && (
                  <span className="px-1.5 sm:px-2 py-0.5 bg-blue-200 text-blue-800 text-[9px] sm:text-[10px] font-bold rounded-full">
                    {checkedInLeadersPct}% with active teams
                  </span>
                )}
              </div>
            </div>
                ...
                <div className="overflow-x-auto -mx-2 sm:mx-0">
                  <table className="min-w-full">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {activeTeamLeaders.map((leader, idx) => {
                        const checkInTime = leader.checkInTime
                          ? new Date(leader.checkInTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })
                          : '—';
                        const checkoutTime = leader.checkoutTime
                          ? new Date(leader.checkoutTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })
                          : '—';

                        // Calculate time spent
                        let timeSpent = '—';
                        if (leader.checkInTime) {
                          const start = new Date(leader.checkInTime).getTime();
                          const end = leader.checkoutTime ? new Date(leader.checkoutTime).getTime() : Date.now();
                        ...
                        return (
                          <tr key={leader.id} className="hover:bg-gray-50">
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-[10px] sm:text-xs text-gray-500 whitespace-nowrap">{idx + 1}</td>
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium text-gray-800 whitespace-nowrap">
                              {leader.name}
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-[10px] sm:text-xs whitespace-nowrap">
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
                            ...
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-[10px] sm:text-xs whitespace-nowrap">
                              {checkoutTime !== '—' ? (
                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-gray-50 text-gray-700 border border-gray-200">
                                  {checkoutTime}
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-yellow-50 text-yellow-700 border border-yellow-100">
                                  Ongoing
                                </span>
                              )}
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-[10px] sm:text-xs text-gray-700 whitespace-nowrap">
                              {timeSpent}
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-[10px] sm:text-xs text-gray-700 whitespace-nowrap">
                              {leader.route_name || '—'}
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden" style={{ maxHeight: '95vh' }}>
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-purple-600 to-indigo-600 flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <h2 className="text-xs sm:text-sm font-bold text-white truncate">Checked-In FMRs</h2>
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 py-2.5 sm:py-3 bg-purple-50 border-b border-purple-100 flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                ...
                <span className="text-[10px] sm:text-xs text-purple-600">FMRs checked in today</span>
            ...
            <div className="overflow-y-auto flex-1 p-2 sm:p-0">
              {loadingActiveReps ? (
                <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                  ...
                  <p className="text-xs sm:text-sm">Loading checked-in FMRs...</p>
                </div>
              ) : activeSalesReps.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                  <AlertCircleIcon className="h-10 w-10 mb-3 opacity-40" />
                  <p className="text-xs sm:text-sm font-medium">No FMRs have checked in today</p>
                </div>
              ) : (
                <div className="overflow-x-auto -mx-2 sm:mx-0">
                  <table className="min-w-full">
